package org.kevoree.android.defaultNodeTypes.osgi.deploy.command
...
import library.defaultNodeTypes.osgi.deploy.command.{StopInstanceCommand, LifeCycleCommand}
    ...
    KevoreeDeployManager.bundleMapping.find(map => map.objClassName == c.getClass.getName && map.name == c.getName) match {
      ...
      case Some(mapfound) => {
        ...
        componentBundle.getRegisteredServices.find({
          ...
          case None => false
          case Some(sr) => {
            val startResult = (componentBundle.getBundleContext.getService(sr).asInstanceOf[KevoreeActor] !? StartMessage(c.getTypeDefinition.eContainer.asInstanceOf[ContainerRoot]))
