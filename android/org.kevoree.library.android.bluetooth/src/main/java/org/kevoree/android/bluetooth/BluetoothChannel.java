...
import org.kevoree.library.ChannelClassResolver;
...
@ChannelTypeFragment
@DictionaryType({
        @DictionaryAttribute(name = "secureUUID", defaultValue = "eae75780-a40f-11e1-b3dd-0800200c9a66", optional = false, fragmentDependant = false),
        @DictionaryAttribute(name = "unsecureUUID", defaultValue = "8a4bc930-9f53-11e1-a8b0-0800200c9a66", optional = false, fragmentDependant = false),
...
public class BluetoothChannel extends AbstractChannelFragment implements BluetoothEventListener {
    ...
    private static final String DEFAULT_SECURE_UUID = "eae75780-a40f-11e1-b3dd-0800200c9a66";
    private static final String DEFAULT_UNSECURE_UUID = "8a4bc930-9f53-11e1-a8b0-0800200c9a66";
    ...
    public void startChannel() {
        ...
        try {
            replay = (getDictionary().get("replay").equals("true")) ? true : false;
        } catch (Exception e) {
            replay = true;
                    ...
                    public void handleMessage(android.os.Message msg) {
                        switch (msg.what) {
                            case Rfcomm.MESSAGE_STATE_CHANGE:
                                switch (msg.arg1) {
                                    case Rfcomm.STATE_CONNECTED:
                                        if (D) Toast.makeText(uiservice.getRootActivity(), "STATE_CONNECTED",
                                                Toast.LENGTH_SHORT).show();
                                        break;
                                    case Rfcomm.STATE_CONNECTING:
                                        if (D) Toast.makeText(uiservice.getRootActivity(), "STATE_CONNECTING",
                                                Toast.LENGTH_SHORT).show();
                                        break;
                                    case Rfcomm.STATE_LISTEN:
                                        if (D) Toast.makeText(uiservice.getRootActivity(), "STATE_LISTEN",
                                                Toast.LENGTH_SHORT).show();
                                        break;
                                    case Rfcomm.STATE_NONE:
                                        if (D) Toast.makeText(uiservice.getRootActivity(), "STATE_NONE",
                                                Toast.LENGTH_SHORT).show();
                                        break;
                                }
                                break;
                            ...
                            case Rfcomm.MESSAGE_DEVICE_NAME:
                                ...
                                if (D) Toast.makeText(uiservice.getRootActivity(), "Connected to "
                                        + connectedDeviceName, Toast.LENGTH_SHORT).show();
                                break;

                            case Rfcomm.MESSAGE_TOAST:
                                if (D) Toast.makeText(uiservice.getRootActivity(), msg.getData().getString(Rfcomm.TOAST),
                                        Toast.LENGTH_SHORT).show();
                                break;
                        }
                    }
