...
/**
 * Utility methods for using Java Reflection APIs to facilitate generic property getter and setter operations on Java objects. Much of this code was
 * originally included in <code>BeanUtils</code>, but has been separated because of the volume of code involved.
 * <p>
 ...
 * <code>PropertyEditor</code> classes that have been registered, although a convenient way to access the registered classes themselves is included.
 * <p>
 * For the purposes of this class, five formats for referencing a particular property value of a bean are defined, with the <i>default</i> layout of
 * an identifying String in parentheses. However the notation for these formats and how they are resolved is now (since BeanUtils 1.8.0) controlled by
 * the configured {@link Resolver} implementation:
 * <ul>
 * <li><strong>Simple (<code>name</code>)</strong> - The specified <code>name</code> identifies an individual property of a particular JavaBean. The
 * name of the actual getter or setter method to be used is determined using standard JavaBeans instrospection, so that (unless overridden by a
 * <code>BeanInfo</code> class, a property named "xyz" will have a getter method named <code>getXyz()</code> or (for boolean properties only)
 * <code>isXyz()</code>, and a setter method named <code>setXyz()</code>.</li>
 * <li><strong>Nested (<code>name1.name2.name3</code>)</strong> The first name element is used to select a property getter, as for simple references
 * above. The object returned for this property is then consulted, using the same approach, for a property getter for a property named
 * <code>name2</code>, and so on. The property value that is ultimately retrieved or modified is the one identified by the last name element.</li>
 * <li><strong>Indexed (<code>name[index]</code>)</strong> - The underlying property value is assumed to be an array, or this JavaBean is assumed to
 * have indexed property getter and setter methods. The appropriate (zero-relative) entry in the array is selected. <code>List</code> objects are now
 * also supported for read/write. You simply need to define a getter that returns the <code>List</code></li>
 * <li><strong>Mapped (<code>name(key)</code>)</strong> - The JavaBean is assumed to have an property getter and setter methods with an additional
 * attribute of type <code>java.lang.String</code>.</li>
 * <li><strong>Combined (<code>name1.name2[index].name3(key)</code>)</strong> - Combining mapped, nested, and indexed references is also supported.</li>
 * </ul>
 * 
...
public class PropertyUtilsBean {
     ...
     * <p>
     * The {@link Resolver} handles the <i>property name</i> expressions and the implementation in use effectively controls the dialect of the
    ...
    /**
     * Clear any cached property descriptors information for all classes loaded by any class loaders. This is useful in cases where class loaders are
    ...
    /**
     * <p>
     * Copy property values from the "origin" bean to the "destination" bean for all cases where the property names are the same (even though the
     * actual getter and setter methods might have been customized via <code>BeanInfo</code> classes). No conversions are performed on the actual
     * property values -- it is assumed that the values retrieved from the origin bean are assignment-compatible with the types expected by the
     * destination bean.
     * </p>
     * 
     * <p>
     * If the origin "bean" is actually a <code>Map</code>, it is assumed to contain String-valued <strong>simple</strong> property names as the keys,
     * pointing at the corresponding property values that will be set in the destination bean.<strong>Note</strong> that this method is intended to
     ...
     *                if the <code>dest</code> or <code>orig</code> argument is null
    ...
    /**
     * <p>
     * Return the entire set of properties for which the specified bean provides a read method. This map contains the unconverted property values for
     * all properties for which a read method is provided (i.e. where the <code>getReadMethod()</code> returns non-null).
     * </p>
     * 
     * <p>
    ...
    /**
     * Return the value of the specified indexed property of the specified bean, with no type conversions. The zero-relative index of the required
     * value must be included (in square brackets) as a suffix to the property name, or <code>IllegalArgumentException</code> will be thrown. In
     * addition to supporting the JavaBeans specification, this method has been extended to support <code>List</code> objects as well.
     * 
     * @param bean
     ...
     *                if the specified index is outside the valid range for the underlying array or List
     * @exception IllegalAccessException
     *                if the caller does not have access to the property accessor method
     * @exception IllegalArgumentException
    ...
    /**
     * Return the value of the specified indexed property of the specified bean, with no type conversions. In addition to supporting the JavaBeans
     * specification, this method has been extended to support <code>List</code> objects as well.
     * 
     * @param bean
     *            Bean whose property is to be extracted
     * @param name
    ...
    /**
     * Return the value of the specified mapped property of the specified bean, with no type conversions. The key of the required value must be
     * included (in brackets) as a suffix to the property name, or <code>IllegalArgumentException</code> will be thrown.
     * 
     * @param bean
     *            Bean whose property is to be extracted
     * @param name
     ...
     * See setPropertyOfMapBean for more information.
     * 
     * @param bean
     *            Map bean
     * @param propertyName
     ...
     *             just in case subclasses override this method to try to access real getter methods and find permission is denied.
     * 
     * @throws InvocationTargetException
     *             just in case subclasses override this method to try to access real getter methods, and find it throws an exception when invoked.
     * 
     * @throws NoSuchMethodException
     *             just in case subclasses override this method to try to access real getter methods, and want to fail if no simple method is
     *             available.
     * @since 1.8.0
     */
    ...
    /**
     * <p>
     * Retrieve the property descriptor for the specified property of the specified bean, or return <code>null</code> if there is no such descriptor.
     * This method resolves indexed and nested property references in the same manner as other methods in this class, except that if the last (or
     * only) name element is indexed, the descriptor for the last resolved property itself is returned.
     * </p>
     * 
     * <p>
     * <strong>FIXME</strong> - Does not work with DynaBeans.
        ...
        for (int i = 0; i < descriptors.length; i++) {
            if (descriptors[i] instanceof IndexedPropertyDescriptor) {
                ...
                if (descriptor.getReadMethod() == null) {
                    String methodName = descriptor.getIndexedReadMethod() != null ? descriptor.getIndexedReadMethod().getName() : "get" + propName;
                ...
                if (descriptor.getWriteMethod() == null) {
                    String methodName = descriptor.getIndexedWriteMethod() != null ? descriptor.getIndexedWriteMethod().getName() : "set" + propName;
    ...
    /**
     * <p>
     * Return the Java Class repesenting the property editor class that has been registered for this property (if any). This method follows the same
     * name resolution rules used by <code>getPropertyDescriptor()</code>, so if the last element of a name reference is indexed, the property editor
     * for the underlying property's class is returned.
     * </p>
     * 
     * <p>
     * Note that <code>null</code> will be returned if there is no property, or if there is no registered property editor class. Because this return
     * value is ambiguous, you should determine the existence of the property itself by other means.
     * </p>
     * 
     * <p>
     * <strong>FIXME</strong> - Does not work with DynaBeans.
    ...
    /**
     * Return the Java Class representing the property type of the specified property, or <code>null</code> if there is no such property for the
     * specified bean. This method follows the same name resolution rules used by <code>getPropertyDescriptor()</code>, so if the last element of a
     * name reference is indexed, the type of the property itself will be returned. If the last (or only) element has no property with the specified
    ...
    /**
     * <p>
     * Return an accessible property getter method for this property, if there is one; otherwise return <code>null</code>.
     * </p>
     * 
     * <p>
     * <strong>FIXME</strong> - Does not work with DynaBeans.
    ...
    /**
     * <p>
     * Return an accessible property setter method for this property, if there is one; otherwise return <code>null</code>.
     * </p>
     * 
     * <p>
     * <strong>FIXME</strong> - Does not work with DynaBeans.
    ...
    /**
     * <p>
     * Return <code>true</code> if the specified property name identifies a readable property on the specified bean; otherwise, return
     * <code>false</code>.
     * 
     * @param bean
    ...
    /**
     * <p>
     * Return <code>true</code> if the specified property name identifies a writeable property on the specified bean; otherwise, return
     * <code>false</code>.
     * 
     * @param bean
     *            Bean to be examined (may be a {@link DynaBean}
    ...
    /**
     * Set the value of the specified indexed property of the specified bean, with no type conversions. The zero-relative index of the required value
     * must be included (in square brackets) as a suffix to the property name, or <code>IllegalArgumentException</code> will be thrown. In addition to
     * supporting the JavaBeans specification, this method has been extended to support <code>List</code> objects as well.
     * 
     * @param bean
    ...
    /**
     * Set the value of the specified indexed property of the specified bean, with no type conversions. In addition to supporting the JavaBeans
     * specification, this method has been extended to support <code>List</code> objects as well.
     * 
     * @param bean
    ...
    /**
     * Set the value of the specified mapped property of the specified bean, with no type conversions. The key of the value to set must be included
     * (in brackets) as a suffix to the property name, or <code>IllegalArgumentException</code> will be thrown.
     * 
     * @param bean
     *            Bean whose property is to be set
     * @param name
    ...
    public void setMappedProperty(Object bean, String name, String key, Object value) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        ...
        if (descriptor instanceof MappedPropertyDescriptor) {
            ...
            if (mappedWriteMethod != null) {
                ...
                if (log.isTraceEnabled()) {
                    String valueClassName = value == null ? "<null>" : value.getClass().getName();
                    log.trace("setSimpleProperty: Invoking method " + mappedWriteMethod + " with key=" + key + ", value=" + value + " (class "
                            + valueClassName + ")");
                }
    ...
    /**
     * This method is called by method setNestedProperty when the current bean is found to be a Map object, and defines how to deal with setting a
     * property on a Map.
     * <p>
     * The standard implementation here is to:
     * <ul>
     * <li>call bean.set(propertyName) for all propertyName values.</li>
     * <li>throw an IllegalArgumentException if the property specifier contains MAPPED_DELIM or INDEXED_DELIM, as Map entries are essentially simple
     * properties; mapping and indexing operations do not make sense when accessing a map (even thought the returned object may be a Map or an Array).
     * </li>
     * </ul>
     * <p>
     * The default behaviour of beanutils 1.7.1 or later is for assigning to "a.b" to mean a.put(b, obj) always. However the behaviour of beanutils
     * version 1.6.0, 1.6.1, 1.7.0 was for "a.b" to mean a.setB(obj) if such a method existed, and a.put(b, obj) otherwise. In version 1.5 it meant
     * a.put(b, obj) always (ie the same as the behaviour in the current version). In versions prior to 1.5 it meant a.setB(obj) always. [yes, this is
     * all <i>very</i> unfortunate]
     * <p>
     * Users who would like to customise the meaning of "a.b" in method setNestedProperty when a is a Map can create a custom subclass of this class
     * and override this method to implement the behaviour of their choice, such as restoring the pre-1.4 behaviour of this class if they wish. When
     * overriding this method, do not forget to deal with MAPPED_DELIM and INDEXED_DELIM characters in the propertyName.
     * <p>
     * Note, however, that the recommended solution for objects that implement Map but want their simple properties to come first is for <i>those</i>
     * objects to override their get/put methods to implement that behaviour, and <i>not</i> to solve the problem by modifying the default behaviour
     * of the PropertyUtilsBean class by overriding this method.
     * 
     * @param bean
     *            Map bean
     * @param propertyName
     ...
     * 
     * @throws IllegalAccessException
     *             just in case subclasses override this method to try to access real setter methods and find permission is denied.
     * 
     * @throws InvocationTargetException
     *             just in case subclasses override this method to try to access real setter methods, and find it throws an exception when invoked.
     * 
     * @throws NoSuchMethodException
     *             just in case subclasses override this method to try to access real setter methods, and want to fail if no simple method is
     *             available.
     * @since 1.8.0
     */
