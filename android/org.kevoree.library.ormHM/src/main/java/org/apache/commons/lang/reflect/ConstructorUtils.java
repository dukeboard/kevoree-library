 ...
 * </p>
 * 
 * <h3>Known Limitations</h3> <h4>Accessing Public Constructors In A Default Access Superclass</h4>
 * <p>
 * There is an issue when invoking public constructors contained in a default access superclass. Reflection locates these constructors fine and
 * correctly assigns them as public. However, an <code>IllegalAccessException</code> is thrown if the constructors is invoked.
 * </p>
 * 
 * <p>
 * <code>ConstructorUtils</code> contains a workaround for this situation. It will attempt to call <code>setAccessible</code> on this constructor. If
 * this call succeeds, then the method can be invoked as normal. This call will only succeed when the application has sufficient security privilages.
...
public class ConstructorUtils {
    ...
    /**
     * <p>
     * Convenience method returning new instance of <code>klazz</code> using a single argument constructor. The formal parameter type is inferred from
     * the actual values of <code>arg</code>. See {@link #invokeExactConstructor(Class, Object[], Class[])} for more details.
     * </p>
     * 
     * <p>
    ...
    /**
     * <p>
     * Returns new instance of <code>klazz</code> created using the actual arguments <code>args</code>. The formal parameter types are inferred from
     * the actual values of <code>args</code>. See {@link #invokeExactConstructor(Class, Object[], Class[])} for more details.
     * </p>
     * 
     * <p>
     * The signatures should be assignment compatible.
    ...
    /**
     * <p>
     * Returns new instance of <code>klazz</code> created using constructor with signature <code>parameterTypes</code> and actual arguments
     * <code>args</code>.
     * </p>
     * 
     * <p>
     ...
     * 
     * <p>
     * First it checks if there is constructor matching the exact signature. If no such, all the constructors of the class are tested if their
     * signatures are assignment compatible with the parameter types. The first matching constructor is returned.
     * </p>
     * 
     * @param cls
    ...
    public static <T> Constructor<T> getMatchingAccessibleConstructor(Class<T> cls, Class<?>[] parameterTypes) {
        ...
        for (Constructor<T> ctor2 : ctors) {
            // compare parameters
            if (ClassUtils.isAssignable(parameterTypes, ctor2.getParameterTypes(), true)) {
                // get accessible version of method
                Constructor<T> ctor = getAccessibleConstructor(ctor2);
                if (ctor != null) {
                    MemberUtils.setAccessibleWorkaround(ctor);
