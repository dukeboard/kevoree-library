...
/**
 * A MappedPropertyDescriptor describes one mapped property. Mapped properties are multivalued properties like indexed properties but that are
 * accessed with a String key instead of an index. Such property values are typically stored in a Map collection. For this class to work properly, a
 * mapped value must have getter and setter methods of the form
 * <p>
...
public class MappedPropertyDescriptor extends PropertyDescriptor {
    ...
    /**
     * Constructs a MappedPropertyDescriptor for a property that follows the standard Java convention by having getFoo and setFoo accessor methods,
     * with the addition of a String parameter (the key). Thus if the argument name is "fred", it will assume that the writer method is "setFred" and
     * the reader method is "getFred". Note that the property name should start with a lower case character, which will be capitalized in the method
     * names.
     * 
     * @param propertyName
     ...
     *            The name of the method used for reading one of the property values. May be null if the property is write-only.
     * @param mappedSetterName
     *            The name of the method used for writing one of the property values. May be null if the property is read-only.
     * 
     * @exception IntrospectionException
     *                if an exception occurs during introspection.
     */
     ...
     *            The programmatic name of the property.
     * @param mappedGetter
     *            The method used for reading one of the property values. May be be null if the property is write-only.
     * @param mappedSetter
     *            The method used for writing one the property values. May be null if the property is read-only.
     * 
     * @exception IntrospectionException
     *                if an exception occurs during introspection.
     */
     ...
     * 
     * @return The Java type info for the property values. Note that the "Class" object may describe a built-in Java type such as "int". The result
     *         may be "null" if this is a mapped property that does not support non-keyed access.
     *         <p>
    ...
    private static Method internalGetMethod(Class initial, String methodName, int parameterCount) {
        ...
        for (Class interface1 : interfaces) {
            Method method = internalGetMethod(interface1, methodName, parameterCount);
            if (method != null) {
                return method;
            }
        }
    ...
    /**
     * Holds a {@link Method} in a {@link SoftReference} so that it it doesn't prevent any ClassLoader being garbage collected, but tries to re-create
