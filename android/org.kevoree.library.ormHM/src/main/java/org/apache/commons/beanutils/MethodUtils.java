 ...
 * assigns them as public. However, an <code>IllegalAccessException</code> is thrown if the method is invoked.
 * </p>
 * 
 * <p>
 * <code>MethodUtils</code> contains a workaround for this situation. It will attempt to call <code>setAccessible</code> on this method. If this call
 * succeeds, then the method can be invoked as normal. This call will only succeed when the application has sufficient security privilages. If this
 * call fails then a warning will be logged and the method may fail.
 * </p>
 * 
...
public class MethodUtils {
     ...
     * <p>
     * Note that this is broken when this class is deployed via a shared classloader in a container, as the warning message will be emitted only once,
     * not once per webapp. However making the warning appear once per webapp means having a map keyed by context classloader which introduces nasty
     * memory-leak problems. As this warning is really optional we can ignore this problem; only one of the webapps will get the warning in its logs
     ...
     * <p>
     * Note that when this class is deployed via a shared classloader in a container, this will affect all webapps. However making this configurable
     * per webapp would mean having a map keyed by context classloader which may introduce memory-leak problems.
     */
     ...
     * <p>
     * The keys into this map only ever exist as temporary variables within methods of this class, and are never exposed to users of this class. This
     * means that the WeakHashMap is used only as a mechanism for limiting the size of the cache, ie a way to tell the garbage collector that the
     * contents of the cache can be completely garbage-collected whenever it needs the memory. Whether this is a good approach to this problem is
     * doubtful; something like the commons-collections LRUMap may be more appropriate (though of course selecting an appropriate size is an issue).
     * <p>
     * This static variable is safe even when this code is deployed via a shared classloader because it is keyed via a MethodDescriptor object which
     * has a Class as one of its members and that member is used in the MethodDescriptor.equals method. So two components that load the same class via
     * different classloaders will generate non-equal MethodDescriptor objects and hence end up with different entries in the map.
     */
     ...
     * 
     * <p>
     * The behaviour of this method is less deterministic than <code>invokeExactMethod()</code>. It loops through all methods with names that match
     * and then executes the first it finds with compatable parameters.
     * </p>
     * 
     * <p>
     * This method supports calls to methods taking primitive parameters via passing in wrapping classes. So, for example, a <code>Boolean</code>
     ...
     * 
     * <p>
     * The behaviour of this method is less deterministic than {@link #invokeExactMethod(Object object,String methodName,Object [] args)}. It loops
     ...
     * {@link #invokeExactMethod(Object object,String methodName,Object [] args,Class[] parameterTypes)}. It loops through all methods with names that
     ...
     * 
     * <p>
     * The behaviour of this method is less deterministic than {@link #invokeExactMethod(Object, String, Object[], Class[])}. It loops through all
     ...
     * <p>
     * The behaviour of this method is less deterministic than
     * {@link #invokeExactStaticMethod(Class objectClass,String methodName,Object [] args,Class[] parameterTypes)}. It loops through all methods with
     ...
     * found, return <code>null</code>. Basically, a convenience wrapper that constructs a <code>Class</code> array for you.
     * </p>
     * 
     * @param clazz
     ...
     * return <code>null</code>. This is just a convenient wrapper for {@link #getAccessibleMethod(Method method)}.
     * </p>
     * 
     * @param clazz
     *            get method from this class
    ...
    /**
     * <p>
     * Return an accessible method (that is, one that can be invoked via reflection) by scanning through the superclasses. If no such method can be
     * found, return <code>null</code>.
     * </p>
     * 
     * @param clazz
    ...
    /**
     * <p>
     * Return an accessible method (that is, one that can be invoked via reflection) that implements the specified method, by scanning through all
     * implemented interfaces and subinterfaces. If no such method can be found, return <code>null</code>.
     * </p>
     * 
     * <p>
     * There isn't any good reason why this method must be private. It is because there doesn't seem any reason why other classes should call this
    ...
    /**
     * <p>
     * Find an accessible method that matches the given name and has compatible parameters. Compatible parameters mean that every method parameter is
     * assignable from the given parameters. In other words, it finds a method with the given name that will take the parameters given.
     * <p>
     * 
     * <p>
     ...
     * 
     * <p>
     * This method can match primitive parameter by passing in wrapper classes. For example, a <code>Boolean</code> will match a primitive
     ...
     * <code>Float</code> to a <code>float</code>, a <code>Integer</code> to a <code>int</code>, and a <code>Double</code> to a <code>double</code>.
     * Now logic widening matches are allowed. For example, a <code>Long</code> will not match a <code>int</code>.
     * 
     * @param parameterType
    ...
    /**
     * Gets the wrapper object class for the given primitive type class. For example, passing <code>boolean.class</code> returns
    ...
    private static class MethodDescriptor {
        ...
        /**
         * Returns the string length of method name. I.e. if the hashcodes are different, the objects are different. If the hashcodes are the same,
