 ...
 * {@link Converter} instance to be used can be registered for each possible destination Class. Unless you override them, standard {@link Converter}
 ...
 * 
 * <p>
 * For backwards compatibility, the standard Converters for primitive types (and the corresponding wrapper classes) return a defined default value
 * when a conversion error occurs. If you prefer to have a {@link ConversionException} thrown instead, replace the standard Converter instances with
 * instances created with the zero-arguments constructor. For example, to cause the Converters for integers to throw an exception on conversion
 * errors, you could do this:
 * </p>
 * 
 * <pre>
 ...
 * 
 * <p>
 * Converters generally treat null input as if it were invalid input, ie they return their default value if one was specified when the converter was
 * constructed, and throw an exception otherwise. If you prefer nulls to be preserved for converters that are converting to objects (not primitives)
 * then register a converter as above, passing a default value of null to the converter constructor (and of course registering that converter only for
 * the .class target).
 * </p>
 * 
 * <p>
 * When a converter is listed above as having no default value, then that converter will throw an exception when passed null or an invalid value as
 * its input. In particular, by default the BigInteger and BigDecimal converters have no default (and are therefore somewhat inconsistent with the
 * other numerical converters which all have zero as their default).
 * </p>
 * 
 * <p>
 * Converters that generate <i>arrays</i> of each of the primitive types are also automatically configured (including String[]). When passed null or
 * invalid input, these return an empty array (not null). See class AbstractArrayConverter for the supported input formats for these converters.
 * </p>
 * 
...
public class ConvertUtilsBean {
    ...
    /**
     * Convert the specified value into a String. If the specified value is an array, the first element (converted to a String) will be returned. The
     * registered {@link Converter} for the <code>java.lang.String</code> class will be used, which allows applications to customize Object->String
    ...
    /**
     * Convert an array of specified values to an array of objects of the specified class (if possible). If the specified Java class is itself an
     * array class, this class will be the type of the returned value. Otherwise, an array will be constructed whose component type is the specified
     * class.
     * 
     * @param values
     ...
     *            <code>true</code> if the converters should throw an exception when a conversion error occurs, otherwise <code>
     * <code>false</code> if a default value should be used.
     * @param defaultNull
     *            <code>true</code>if the <i>standard</i> converters (see {@link ConvertUtilsBean#registerStandard(boolean, boolean)}) should use a
     *            default value of <code>null</code>, otherwise <code>false</code>. N.B. This values is ignored if <code>throwException</code> is
     *            <code>true</code>
     * @param defaultArraySize
     *            The size of the default array value for array converters (N.B. This values is ignored if <code>throwException</code> is
     *            <code>true</code>). Specifying a value less than zero causes a <code>null<code> value to be used for
     * the default.
     */
    ...
    /**
     * Register the converters for primitive types. </p> This method registers the following converters:
     * <ul>
    ...
    /**
     * Register the converters for standard types. </p> This method registers the following converters:
     * <ul>
    ...
    /**
     * Register the converters for other types. </p> This method registers the following converters:
     * <ul>
    ...
    /**
     * Register a new ArrayConverter with the specified element delegate converter that returns a default array of the specified size in the event of
     * conversion errors.
     * 
     * @param componentType
     ...
     * @param throwException
     *            Whether a conversion exception should be thrown or a default value used in the event of a conversion error
     * @param defaultArraySize
    ...
    /**
     * Look up and return any registered {@link Converter} for the specified destination class; if there is no registered Converter, return
     * <code>null</code>.
     * 
     * @param clazz
